...
Vendored unchanged from the `resolvability` release: the groundwater archive is
generated against this exact basis, so rendering its posterior needs the same
one to map the 100 KL coefficients back to a field. It implements the Gaussian
prior of Beskos, Girolami, Lan, Farrell and Stuart (2016), Sec. 4.2,
Eqs. (37)-(38): a cosine eigenbasis on the unit square with Whittle-Matern-type
spectral decay.

Modes are the multi-index ``k = (i1, i2)`` with ``i1, i2 in {1, ..., K}``, giving ``d = K^2``:

    phi_k(x1, x2) = 2 cos(pi (i1 + 1/2) x1) cos(pi (i2 + 1/2) x2),               (Eq. 37)
    lambda_k^2    = sigma^2 ( alpha + pi^2 ((i1+1/2)^2 + (i2+1/2)^2) )^{-s}.     (Eq. 38)

``lambda_k^2`` is the covariance eigenvalue, so the KL amplitude is ``amp_k = lambda_k`` and a
whitened standard-normal latent maps to the field by ``u(x) = sum_k amp_k xi_k phi_k(x)``. The
defaults are Stuart's groundwater hyper-parameters, ``alpha = 0, s = 1.1, sigma = 1, K = 10``,
i.e. d = 100 unknowns. The basis is evaluated on the same vertex grid the solver uses
(``x_i = i/(N-1)``, endpoints included), so ``reconstruct(xi)`` returns field values at exactly
the nodes the Darcy operator reads.
"""
